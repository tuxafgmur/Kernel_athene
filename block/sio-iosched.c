 ...
 * aleatory access devices, but it does some basic merging.
 * We try to keep minimum overhead to achieve low latency.
 *
 * Asynchronous and synchronous requests are not treated separately,
 * but we relay on deadlines to ensure fairness.
 *
 * This version incorporates several fixes and logic improvements.
 */
